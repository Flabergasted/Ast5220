...
ax.set_title("Free electron fraction in the universe")
...
fig, ax = plt.subplots()
fig.set_size_inches(10, 7.5)
ax.set_title("The visibility function and its derivatives")
...
ax.set_xlabel("x")
ax.set_ylabel(r"$\tilde{g} (x)$, $\tilde{g}' (x)$, $\tilde{g}'' (x)$")
ax.legend()
#ax.set_ylim(ymin = 1e-8, ymax = 1e8)
...
print("Sound horizon: ",s_of_x[np.argmin(np.abs(tau_of_x-1))]/Mpc, " Mpc")
print(np.min(np.abs(tau_of_x-1)))