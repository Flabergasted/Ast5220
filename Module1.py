...
"""Hprad = H0 * np.sqrt(OmegaR_of_x + OmegaNu_of_x) * np.exp(-x)
Hpmat = H0 * np.sqrt(OmegaB_of_x + OmegaCDM_of_x) * np.exp(-1/2 * x)
HpDE = H0 * np.sqrt(OmegaLambda_of_x) * np.exp(x)"""
print("Hp")
fig, ax = plt.subplots()
fig.set_size_inches(10, 7.5)
...
ax.plot()
"""ax.plot(x, Hprad *Mpc/(100*1000), ls = "--", color = "k", label = "Radiation domination")
ax.plot(x, Hpmat *Mpc/(100*1000), ls = "--", color = "r", label = "Matter domination")
ax.plot(x, HpDE *Mpc/(100*1000), ls = "--", color = "b", label = "Dark energy domination")"""
ax.semilogy()
ax.set_xlabel("x = log(a)")
...
"""ax.plot(x, eta*Hprad/const.c, ls = "--", color = "k", label = "Radiation domination")
ax.plot(x, eta*Hpmat/const.c, ls = "--", color = "r", label = "Matter domination")
ax.plot(x, eta*HpDE/const.c, ls = "--", color = "b", label = "Dark energy domination")"""
ax.set_xlabel("x = log(a)")
...
ax.errorbar(z, d_L_data/z, yerr=SN_errorbars, fmt=".", capsize=3, color="k", label = "data")
a_temp = np.exp(x)
z_temp = 1/a_temp - 1
...
fig.set_size_inches(10, 7.5)
#ax[0,0].vspan("lower threshold hubble")
ax[0,0].axvline(0.67, ls="--", color="k", label="Planck (2018) best-fit value")
#ax[0,0].vspan("upper threshold hubble")
#ax[0,0].vlines(0.67, ymin=0, ymax=1000, ls="--", color="k", label="Planck (2018) best-fit value")
#ax[0,0].axvline("Hubble const.", ls="--", color="k", label="Planck (2018) best-fit value")
